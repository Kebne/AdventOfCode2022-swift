...
final class SixteenTests: XCTestCase {
    let sut = Sixteen()

    func testFirstExample() throws {
        XCTAssertEqual(sut.run(input: testInput), 1651)
    }

    func testFirst() throws {
        XCTAssertEqual(sut.run(input: input), 1673)
    }

    func testSecondExample() {
        XCTAssertEqual(sut.runParallell(input: testInput), 1707)
    }

    func testSecond() {
        XCTAssertEqual(sut.runParallell(input: input), 2343)
    }
}

extension SixteenTests {
