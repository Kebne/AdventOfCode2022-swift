...
final class FifthteenTests: XCTestCase {
    let sut = Fifthteen()

    func testFirstExample() throws {
        XCTAssertEqual(sut.countImpossibles(in: testInput, at: 9), 25)
        XCTAssertEqual(sut.countImpossibles(in: testInput, at: 10), 26)
        XCTAssertEqual(sut.countImpossibles(in: testInput, at: 11), 27)
    }

    func testFirst() throws {
        XCTAssertEqual(sut.countImpossibles(in: input, at: 2000000), 4502208)
    }

    func testSecondExample() {
        XCTAssertEqual(sut.findBeacon(in: testInput, lower: 0, upper: 20), 56000011)
    }

    func testSecond() {
        XCTAssertEqual(sut.findBeacon(in: input, lower: 0, upper: 4000000), 13784551204480)
    }
}

extension FifthteenTests {
